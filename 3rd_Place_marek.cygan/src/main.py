...
def read_csv(valid_seed, reverse_order, adir, bdir):
...
def extract_names(names):
    return map(lambda (x,_i,_j,_k,_l, _m) : x.split('/')[-3]+'_'+x.split('/')[-1][:-4], names)
...
def replicate(mylist, pred_iter):
    newlist = []
    for xx in mylist:
        for _ in range(pred_iter):
            newlist.append(xx)
    return newlist
...
def train(args):
    ...
    print 'device', device
    valid_list, train_list, test_list = read_csv(args.valid_seed, args.reverse_order, 'example_extracted', args.test_dir)
    ...
    with tf.device(device):
        ...
        if not args.predict or args.predict[0] == '1':
            train_batch = tf_batch(train_list, batch_size=args.batch_size)

        if not args.predict or args.predict[1] == '1':
            valid_list = replicate(valid_list, args.pred_iter)
            valid_batch = tf_batch(valid_list, batch_size=args.batch_size, shuffle=False)

        if args.predict and args.predict[2] == '1':
            test_list = replicate(test_list, args.pred_iter)
            test_batch = tf_batch(test_list, batch_size=args.batch_size, shuffle=False)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            if args.predict:
                ...
                for (batch_gen, namelist, pref, pt) in pass_list:
                    ...
                    losses = []
                    while tot_cnt < size:
                        print 'tot_cnt', tot_cnt, 'pred_dict_size', len(pred_dict)
