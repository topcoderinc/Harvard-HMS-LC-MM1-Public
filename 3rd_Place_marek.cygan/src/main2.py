...
def load_valid_heatmaps(validlist):
    ...
    first = True
    for (name, (_, _, _, heatmap_path, _)) in zip(namelist, validlist):
...
def my_read_image(x):
    ...
    #TODO crop
    if dx > 0:
        heatmap = heatmap[dx:CROP_SIZE+dx, dx:CROP_SIZE+dx, :]
        img = img[dx:CROP_SIZE+dx, dx:CROP_SIZE+dx, :]
    elif dx < 0:
        newheat = np.zeros((CROP_SIZE, CROP_SIZE, OUT_CHANNELS), dtype=np.float32)
        newimg = np.zeros((CROP_SIZE, CROP_SIZE, NUM_CHANNELS), dtype=np.float32)
        newheat[-dx:CROP_SIZE+dx, -dx:CROP_SIZE+dx, :] = heatmap
        newimg[-dx:CROP_SIZE+dx, -dx:CROP_SIZE+dx, :] = img
        heatmap = newheat
        img = newimg
...
def estimate_mean(sess, single_batch, sampled_batches=20):
    ...
    for batchid in range(sampled_batches):
        names, im, heat, _, _ = sess.run(single_batch)
    ...
    meanheat = np.log(meanheat) * meanheat + (1.0 - meanheat) * np.log(1.-meanheat)
    print mean
    print 'meanheat', meanheat

    var = None
    for _ in range(sampled_batches):
        _, im, _, _, _ = sess.run(single_batch)
        print 'has batch'
...
def tf_batch(file_paths, batch_size, shuffle=True):
    ...
    image, heatmap, mask, aug_data = tf.py_func(my_read_image, [input_queue[0]], [tf.float32, tf.float32, tf.float32, tf.int64])
    ...
    shape_list = [(5), (CROP_SIZE, CROP_SIZE, NUM_CHANNELS), (FINAL_SIZE, FINAL_SIZE, OUT_CHANNELS), (OUT_CHANNELS), (1)]
    print 'shape_list', shape_list
    single_batch = tf.train.batch([input_queue[0], image, heatmap, mask, aug_data], batch_size=batch_size,
                                  num_threads=2 if LAPTOP else 6, capacity=6*batch_size,
                                  shapes=shape_list)
    return single_batch
...
def make_train_list(patients, dict, dir, dict_mask, has_heat=True):
    res = []
    for p in patients:
        assert(p in dict)
        scans = dict[p]
        for i in range(1,len(scans)-1):
            ...
            res.append((image_path, image_path0, image_path2, heatmap_path, mask))
    return res
...
def read_csv():
    ...
    return make_train_list(patients[:valid_size], dict, adir, dict_mask), \
           make_train_list(patients[valid_size:], dict, adir, dict_mask), \
           make_train_list(patients_test, dict_test, bdir, dict_mask=None, has_heat=False)
...
def extract_names(names):
    return map(lambda (x,_i,_j,_k,_l) : x.split('/')[1]+'_'+x.split('/')[3][:-4], names)
    ...
    with tf.device(device):
        ...
        if args.dilarch == 0:
            ...
            dilarch = [(8, 1, 3, 0), (8, 1, 3, 1),
                       ...
                       (32, 1, 3, 0), (32, 1, 3, 0),
                       (64, 2, 3, 0), (64, 2, 3, 0),
                       (128, 4, 3, 0), (128, 4, 3, 0)] + args.num_dense * [(args.neuron_dense, 1, 1, 0)] + [(OUT_CHANNELS, 1, 1, 0)]
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            if args.predict:
                ...
                pass_list = [
                     #        (train_batch, len(train_list) - 13 * args.batch_size, 'train', True),
                     #        (train_batch, len(train_list), 'train', False),
                ...
                for (batch_gen, namelist, pref, pt) in pass_list:
                    ...
                    while tot_cnt < size * args.pred_iter:
                        ...
                        for i in range(args.batch_size):
                            name = names[i]
                            if nameset[name] < args.pred_iter:
                                nameset[name] += 1
                                tot_cnt += 1
                                if not pt:
                                    [dx] = aug_data[i]
            ...
            while epoch < args.num_epochs:
                ...
                for compute_list, num_batches, batch_gen, pt in pass_list:
                    ...
                    for i in range(num_batches):
                        ...
                        _, batch_x, batch_heat, batch_mask, _ = sess.run(batch_gen)
                        if epoch == 1:
                        ...
                        myfeed = {x: batch_x, y_heat: batch_heat, phase_train: pt, learning_rate: epoch_lr,
                                  coefficients: np.array([1] + (OUT_CHANNELS-1) * [0.05]), mask: batch_mask}
...
def create_parser():
    ...
    parser.add_argument('--min_size', type=int, default=300)
    parser.add_argument('--max_size', type=int, default=330)
