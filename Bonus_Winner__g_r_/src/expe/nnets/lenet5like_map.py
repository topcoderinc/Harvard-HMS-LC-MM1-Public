...
conv2_weights = tf.Variable(tf.truncated_normal(
    [5, 5, 32, 16], stddev=0.1,
    seed=SEED, dtype=data_type()))
conv2_biases = tf.Variable(tf.constant(0.1, shape=[16], dtype=data_type()))
fc1_weights = tf.Variable(  # fully connected, depth 512.
    tf.truncated_normal([SIZE4 * SIZE4 * 16, 512],
                        stddev=0.1,
                        seed=SEED,
                        dtype=data_type()))
fc1_biases = tf.Variable(tf.constant(0.1, shape=[512], dtype=data_type()))
fc2_weights = tf.Variable(tf.truncated_normal([512, TARGET_SIZE*TARGET_SIZE],
                                              stddev=0.1,
                                              seed=SEED,
                                              dtype=data_type()))
fc2_biases = tf.Variable(tf.constant(
    0.1, shape=[TARGET_SIZE*TARGET_SIZE], dtype=data_type()))
...
def model(data_in, train=False):
    ...
    return tf.reshape(output, [batch_size, TARGET_SIZE, TARGET_SIZE], "reshape2")
