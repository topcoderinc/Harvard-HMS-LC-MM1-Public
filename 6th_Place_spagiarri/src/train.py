...
def train(H, test_images):
    ...
    for phase in ['train', 'test']:
        ...
        #dtypes = [tf.float32, tf.float32] - maybe it was ok
        grid_size = H['grid_width'] * H['grid_height']
        shapes = (
        ...
        # Reshape to 3 color image.
        #print x_in
        #x_in = np.reshape(x_in, (H['image_height'], H['image_width'], 1))
        enqueue_op[phase] = q[phase].enqueue((x_in, confs_in, boxes_in))

    def make_feed(d):
        x = np.reshape(d['image'], (H['image_height'], H['image_width'], 1))
        new_x = np.zeros((H['image_height'], H['image_width'], 3))
        for first in range(0, H['image_height']):
            for second in range(0, H['image_width']):
                new_x[first][second] = [x[first][second], x[first][second], x[first][second]]
        #print x
        #print new_x
        return {x_in: new_x, confs_in: d['confs'], boxes_in: d['boxes'],
    ...
    with tf.Session(config=config) as sess:
        ...
        start = time.time()
        max_iter = H['solver'].get('max_iter', 10000)
