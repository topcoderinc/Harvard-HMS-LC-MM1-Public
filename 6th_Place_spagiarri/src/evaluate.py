...
from utils.annolist.AnnotationLib import AnnoRect as r
        ...
        for i in range(len(true_annolist)):
            ...
            for i in range(0, len(rects)):
                if rects[i].score > 0.1:
                    ...
                    print "%s %s %s %s %s" % (rects[i].x1, rects[i].x2, rects[i].y1, rects[i].y2, rects[i].score)
...
def main():
    ...
    for anno in pred_annolist:
        print r.writeJSON(anno.rects)
    #for i in range(0, len(rects)):
    #            if rects[i].score > 0.5:
    #                print pred_anno
    #                print r.writeJSON(rects[i])
