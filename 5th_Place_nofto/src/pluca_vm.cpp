...
vector <double> THRESHOLD = {0.14}; //0.02, 0.04, 0.06, 0.08, 0.09, 0.1, 0.11, 0.12, 0.13, 0.14, 0.145, 0.15, 0.155, 0.16, 0.165, 0.17, 0.18, 0.19, 0.2, 0.25};
...
int MAXSAMPLESIZE = 300000; //10000000;
...
int main(int argc, char* argv[]){
	...
	int vertices = argc > 6 ? atoi(argv[6]) : 100; //30;
	string run = argc > 7 ? argv[7] : "";

	TREES = argc > 8 ? atoi(argv[8]) : 500;
	MAXSAMPLESIZE = argc > 9 ? atoi(argv[9]) : 300000;
	...
	string modelFolder = argc > 4 + pre ? argv[4 + pre] : "";
	...
	if(modelFolder != "" && modelFolder.back() != '/') modelFolder.push_back('/');
	...
	if(phase == "train") cout << "    Train files: " << trainFiles.size() << endl;
	else cout << "    Test files: " << testFiles.size() << endl;
	...
	if(phase == "test" || run == "1"){
		ifstream in((modelFolder + "traindata.csv").c_str());
		string line;
		int ln = 0;
		string prevId = "";
		int num = 0;
		...
		clog << "    " << trainData.size() << " polygons found in traindata." << endl;
		in.close();
		...
		double tic = getTime();
		clog << "    Building random forest ..." << endl;
		for(int j = 0; j < TREES; j++){
			randomForest[j] = USEMSE ? buildTreeMSE() : buildTree();
			if(j % 1 == 0) clog << "    " << j + 1 << " / " << TREES << " trees done ...\r";
		}
		clog << endl;
	...
	for(int i = 0; i < outFiles; i++) out[i].open((outputFile + (i == 0 ? "" : SSTR(i))).c_str());
	...
	for(int of = 0; of < outFiles; of++){
		...
		clog << "    Number of returned polygons: " << returnedSegments[of] << " / " << totalSegments << " = " << double(returnedSegments[of])/totalSegments << endl;
	}
	//clog << endl;
	clog << "    Testing time: " << getTime() - start << " sec." << endl;
